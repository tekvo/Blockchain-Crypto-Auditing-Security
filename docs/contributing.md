---
title: Contributing
description: How maintainers extend this site — tone, industry pages, MkDocs, hooks, Render deployment, and the code library — Tekvo.
---
...
- **Unique `description` (and optional `title`)** in YAML front matter at the top of each page drives `<meta name="description">`, Open Graph, and Twitter cards (see `overrides/main.html`).
- **`site_url`** in `mkdocs.yml` must match the public hostname (e.g. `https://blockchain-sec.tekvo.io/`).
- **`mkdocs_hooks.py`** writes **`sitemap.xml`** after each build; **`docs/robots.txt`** references it.
- Industry pages generated by `scripts/generate_industry_pages.py` already include **`description`** in front matter.
